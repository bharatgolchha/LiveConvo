...
export const SetupModal: React.FC<SetupModalProps> = ({
  ...
  // Memoize conversation type options to prevent re-creation
  const conversationTypeOptions = useMemo(() => [
    { value: 'sales', label: 'Sales Call', icon: Target, color: 'text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800' },
    { value: 'support', label: 'Support Call', icon: Users, color: 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800' },
    { value: 'meeting', label: 'Meeting', icon: Briefcase, color: 'text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/30 border-purple-200 dark:border-purple-800' },
    { value: 'interview', label: 'Interview', icon: Phone, color: 'text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/30 border-orange-200 dark:border-orange-800' },
  ], []);

  // Memoize filtered sessions to prevent expensive filtering on every render
  ...
  }, [sessions, previousConversationSearch]);

  // Memoize file validation to prevent re-creation
  ...
  const handleEnhancedFileUpload = useCallback((newFiles: FileList | null) => {
    if (!newFiles) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ref={modalRef}
        className="relative bg-card rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-border/50"
        tabIndex={-1}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {/* Tab Navigation */}
        <div className="flex-shrink-0 bg-muted/70 border-b border-border/50">
          <div className="flex">
            {[
              ...
              <button
                ...
                className={cn(
                  ...
                  activeTab === id
                    ? "text-blue-600 bg-card/80 border-b-2 border-blue-500 shadow-sm"
