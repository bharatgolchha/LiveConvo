...
export async function exportSummaryToPDF(options: SummaryExportOptions): Promise<void> {
  ...
  doc.text('LiveConvo', margin, yPosition);
  
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(107, 114, 128); // Gray color
  doc.text('AI Meeting Summary', margin, yPosition + 8);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    // Footer text
    doc.text('Generated by LiveConvo - Your AI Meeting Assistant', margin, 285);
  }
...
export function exportSummaryToText(options: SummaryExportOptions): void {
  ...
  content += `\n---\nGenerated by LiveConvo - Your AI Meeting Assistant`;
...
export function exportSummaryToJSON(options: SummaryExportOptions): void {
  ...
  const exportData = {
    metadata: {
      ...
      generator: 'LiveConvo - AI Meeting Assistant'
    },
    summary: {
      tldr: summary.tldr,
