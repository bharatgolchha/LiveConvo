...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: AddSeatsRequest = await request.json();
    ...
    const newQuantity = subscription.quantity + additionalSeats;

    // Check maximum seats limit
    if (subscription.plans.team_maximum_seats && newQuantity > subscription.plans.team_maximum_seats) {
      return NextResponse.json({ 
        error: `Cannot exceed maximum of ${subscription.plans.team_maximum_seats} seats`,
        currentSeats: subscription.quantity,
        maxSeats: subscription.plans.team_maximum_seats
      }, { status: 400 });
    }
    ...
    console.error('Error in add seats endpoint:', error);
    return NextResponse.json(
